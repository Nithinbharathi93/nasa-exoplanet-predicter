...
export default function ExoplanetForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Simulate network delay to see the loading screen
  ...
  const fields = [
    ...
    { name: "transit_depth", label: "TRANSIT DEPTH (0-1)" },
  ...
  const renderContent = () => {
    if (loading) {
      return (
        <div className="flex flex-col items-center justify-center text-center">
          ...
          <h2 className="text-2xl mt-8 tracking-widest text-glow">TRAVEL ARRANGEMENTS IN PROGRESS</h2>
    ...
    if (error) {
      return (
        <div className="text-center p-8 border-2 border-red-500/50 bg-red-500/10">
          <h2 className="text-2xl text-red-400 text-glow tracking-widest">TRANSMISSION ERROR</h2>
    ...
    if (result) {
      return (
        <div className="text-center p-8 border-2 border-emerald-500/50 bg-emerald-500/10">
          <h2 className="text-2xl text-emerald-400 text-glow tracking-widest">PREDICTION RECEIVED</h2>
          <p className="text-4xl font-bold mt-4 text-white">{result.prediction}</p>
          <button
            onClick={() => setResult(null)}
            className="mt-6 bg-transparent border-2 border-emerald-400 px-6 py-2 hover:bg-emerald-400 hover:text-black transition-all duration-300"
          >
            NEW CALCULATION
          </button>
        </div>
